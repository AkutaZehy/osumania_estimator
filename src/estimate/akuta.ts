// ============================================================
// estimate/akuta.ts — AKUTA difficulty estimate (v4)
//
// RC channel:
...
/**
 * Pull model features straight from structured result fields. Jack segments
 * grade on (gridTotalNotes, gridTotalNotes); stream segments are run-level
 * and grade on the mean density per row (SegmentResult.avgPerRow).
 */
export function extractFeatures(r: DifficultyResult): Features {
  const c = r.custom;
  const ga = r.gridAnalysis;
  const cells: CellResult[] = ga ? ga.cells : [];
  ...
  return {
    ...
    techTrills24: c.tech.rollTrill.trills24,
